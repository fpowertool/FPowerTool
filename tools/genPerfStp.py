...
probe perf.hw.bus_cycles.$1.counter("bus_cycles"){} 
...
probe perf.hw.ref_cpu_cycles.$1.counter("ref_cpu_cycles"){} 
		...
		while line:
			...
			try:
				if int(s[1])< int(argv[1]) and s[0]!='_start':
					...
					f.write('probe $1.function("'+s[0]+'").call{  printf("'+s[0]+',1,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d \\n", @perf("branch_instructions"),@perf("branch_misses"),@perf("bus_cycles"),@perf("cache_misses"),@perf("cache_references"),@perf("cpu_cycles"),@perf("instructions"),@perf("ref_cpu_cycles"),@perf("cpu_clock"),@perf("page_faults_min"),@perf("task_clock") ); }\n')
