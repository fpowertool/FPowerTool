...
def main(argv): #argv[1]
	...
	print('fn\tbranch_instructions\tbranch_misses\tbus_cycles\tcache_misses\tcache_references\tcpu_cycles\tinstructions\tref_cpu_cycles\tcpu_clock\tpage_faults_min\ttask_clock')
	parentlinetmp=1
			...
			if	j.split(',')[1]=='1': #不是1就是2啊
					...
					if chongming==0:
							...
							if parentID<0:
								...
								tmpdict={'id':(i),'name':funname,'branch_instructions':data2kexue(int(j.split(',')[2])-int(tmpline.split(',')[2])),'branch_misses':data2kexue(int(j.split(',')[3])-int(tmpline.split(',')[3])),'bus_cycles':data2kexue(int(j.split(',')[4])-int(tmpline.split(',')[4])),'cache_misses':data2kexue(int(j.split(',')[5])-int(tmpline.split(',')[5])),'cache_references':data2kexue(int(j.split(',')[6])-int(tmpline.split(',')[6])),'cpu_cycles':data2kexue(int(j.split(',')[7])-int(tmpline.split(',')[7])),'instructions':data2kexue(int(j.split(',')[8])-int(tmpline.split(',')[8])),'ref_cpu_cycles':data2kexue(int(j.split(',')[9])-int(tmpline.split(',')[9])),'cpu_clock':data2kexue(int(j.split(',')[10])-int(tmpline.split(',')[10])),'page_faults_min':data2kexue(int(j.split(',')[11])-int(tmpline.split(',')[11])),'task_clock':data2kexue(int(j.split(',')[12])-int(tmpline.split(',')[12]))			}

							else:
								tmpdict={'id':(i),'name':funname,'branch_instructions':data2kexue(int(j.split(',')[2])-int(tmpline.split(',')[2])),'branch_misses':data2kexue(int(j.split(',')[3])-int(tmpline.split(',')[3])),'bus_cycles':data2kexue(int(j.split(',')[4])-int(tmpline.split(',')[4])),'cache_misses':data2kexue(int(j.split(',')[5])-int(tmpline.split(',')[5])),'cache_references':data2kexue(int(j.split(',')[6])-int(tmpline.split(',')[6])),'cpu_cycles':data2kexue(int(j.split(',')[7])-int(tmpline.split(',')[7])),'instructions':data2kexue(int(j.split(',')[8])-int(tmpline.split(',')[8])),'ref_cpu_cycles':data2kexue(int(j.split(',')[9])-int(tmpline.split(',')[9])),'cpu_clock':data2kexue(int(j.split(',')[10])-int(tmpline.split(',')[10])),'page_faults_min':data2kexue(int(j.split(',')[11])-int(tmpline.split(',')[11])),'task_clock':data2kexue(int(j.split(',')[12])-int(tmpline.split(',')[12]))		,'_parentId':parentID}


							#################tmpline和j间的数据
							funPerfList.append(tmpdict)
							print(tmpdict['name']+'\t'+tmpdict['branch_instructions']+'\t'+tmpdict['branch_misses']+'\t'+tmpdict['bus_cycles']+'\t'+tmpdict['cache_misses']+'\t'+tmpdict['cache_references']+'\t'+tmpdict['cpu_cycles']+'\t'+tmpdict['instructions']+'\t'+tmpdict['ref_cpu_cycles']+'\t'+tmpdict['cpu_clock']+'\t'+tmpdict['page_faults_min']+'\t'+tmpdict['task_clock'])
