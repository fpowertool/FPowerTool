...
date +%s_%N >>time4.log && likwid-pin -c 22   $* && date +%s_%N >>time4.log  
#date +%s_%N >>time4.log &&    $* && date +%s_%N >>time4.log  

sleep 4
echo "stop fpowertool..."
ps -efww|grep -w 'raplpapi'|grep -v grep|cut -c 9-19|xargs kill -9
sleep 2
